...
def clear_all_app_settings(parent, palette):
    is_confirmed = show_custom_message(parent, "Confirm Settings Reset", "Are you sure you want to reset all application settings to default?\n(This does NOT erase test data).", "confirm", palette)
    if is_confirmed:
        show_custom_message(parent, "Settings Cleared", "Mock: All application settings have been reset.", "success", palette)
...
def erase_all_report_data(parent, palette, reload_callback=None):
    is_confirmed = show_custom_message(parent, "Confirm Deletion", "Are you sure you want to permanently erase ALL PoultriScan test data?\nThis action cannot be undone.", "confirm", palette)
    if is_confirmed:
        try:
            if os.path.exists(REPORT_FILE):
                os.remove(REPORT_FILE)
                show_custom_message(parent, "Deletion Successful", "All test data has been erased.", "success", palette)
            else:
                show_custom_message(parent, "No Data Found", "Report file does not exist. Nothing to erase.", "info", palette)
            if reload_callback:
                reload_callback() 
        except Exception as e:
            show_custom_message(parent, "Deletion Error", f"Failed to erase file: {e}", "error", palette)
...
def create_settings_tab(tab_control, main_window, theme_switch_callback, palette, themes_dict, root_window, reload_reports_callback=None):
    ...
    btn_reset_settings = QPushButton(" CLEAR SETTINGS")
    btn_reset_settings.setObjectName("secondary")
    btn_reset_settings.setIcon(qta.icon('fa5s.cog', color=unselected_text_color))
    btn_reset_settings.clicked.connect(lambda: clear_all_app_settings(settings_tab_content, palette))
    mgmt_layout.addWidget(btn_reset_settings)
