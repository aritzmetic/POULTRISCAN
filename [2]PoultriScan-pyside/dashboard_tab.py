...
DATABASE_LOG_FILE = "raw_database_log.csv" # <-- File definition
...
class DashboardTab(QWidget):
    ...
    # --- Function to save to database log ---
    def save_to_database_log(self, sample_id, raw_readings):
        ...
        # Define the header to match your ERD (tbl_reading)
        header = (
        ...
        try:
            # Map the data from the raw_readings dict to the ERD columns
            data_row = [
                sample_id,
    ...
    def update_gui_and_archive(self, raw_readings):
        ...
        try:
            # --- FIX: This is where the indices are calculated from the raw_readings (which contain AS7265X_ch... keys) ---
        ...
        # --- FIX: This function now takes the calculated indices as arguments ---
        def _update_raw_labels(whc_val, fac_val, myo_val):
            ...
            # --- FIX: Display the calculated indices passed as arguments ---
            # The "Raw" card will now show the calculated index. This seems to be the original intent.
        ...
        # --- FIX: Pass the calculated indices to the _update_raw_labels function ---
        ...
        def _show_archive_dialog():
            ...
            if is_confirmed_to_save:
                ...
                # --- FIX: Build the readings_list using the *calculated indices* ---
                readings_list = [
                    raw_readings.get("Temperature", 'N/A'),
                    raw_readings.get("Humidity", 'N/A'),
                    whc_index,  # Use calculated value
                    fac_index,  # Use calculated value
                    myoglobin_index, # Use calculated value
                    raw_readings.get("MQ-137 (Ammonia)", 'N/A'),
                    raw_readings.get("MQ-135 (Air Quality)", 'N/A'),
                    raw_readings.get("MQ-7 (CO)", 'N/A'),
                    raw_readings.get("MQ-4 (Methane)", 'N/A')
